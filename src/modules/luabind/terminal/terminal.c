...
#define METATABLE_NAME   "st_terminal"
...
void *st_module_luabind_terminal_get_func(const char *func_name) {
    st_modfuncstbl_t *funcs_table = &st_module_luabind_terminal_funcs_table;
...
st_moddata_t *st_module_luabind_terminal_init(st_modsmgr_t *modsmgr,
 st_modsmgr_funcs_t *modsmgr_funcs) {
    ...
    if (err) {
        ...
        fprintf(stderr, "Unable to init module \"luabind_terminal\": %s\n",
         err_msg_buf);
    ...
    return &st_module_luabind_terminal_data;
}
...
static bool st_luabind_import_functions(st_modctx_t *luabind_ctx,
 st_modctx_t *lua_ctx, st_modctx_t *logger_ctx) {
    ...
    ST_LOAD_GLOBAL_FUNCTION("luabind_terminal", lua, register_cfunction);
...
static void st_luabind_bind_all(st_modctx_t *luabind_ctx) {
    ...
    st_lua_register_cfunction(lua_state, METATABLE_NAME, st_terminal_init_bind);
    st_lua_create_metatable(lua_state, METATABLE_NAME);

    st_lua_set_cfunction_to_field(lua_state, "__gc", st_terminal_quit_bind);
    st_lua_set_cfunction_to_field(lua_state, "quit", st_terminal_quit_bind);
